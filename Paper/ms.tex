...
As a result, we are able to sample the posterior distribution for $N$ in
a single run which takes about 15 minutes, whereas the approach of
\citep{2011MNRAS.410...94G} takes approximately 30 minutes per planet
(Gregory, priv. comm.).
...
To fit a planet model to radial velocity data, we need parameters to describe
the properties of each planet. We also include
...
The marginal likelihood was $\ln(\mathcal{Z}) \approx -217.04$ and the
information was $\mathcal{H} \approx 80.0$ nats.
...
information was $\mathcal{H} \approx 140.2$ nats.
