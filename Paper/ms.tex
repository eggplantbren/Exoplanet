...
parameter $s_0$ is an ``extra noise'' parameter that effectively increases the
...
of the ``information'', or Kullback-Leibler divergence from the prior to the
posterior, which quantifies how much we learned about the parameters:
\begin{eqnarray}
...
For the $\nu$ Oph data the information was $\mathcal{H} \approx 77.7$ nats or
111 bits, so the posterior is roughly $e^{-77}$ times the volume of the prior.
