...
$\phi_i$ (defined such that $\psi=0$ gives an RV signal whose maximum is at
$t=0$), the eccentricity $e_i$, and the ``viewing angle'' $\theta_i$
(also known as the longitude of the line of sight).
...
and $\psi_i = \{P_i, A_i, e_i, \phi_i, \theta_i\}$
...
requires a ``viewing angle'' parameter $\theta$ also known as the longitude
