...
We assigned hierarchical priors to the planet's parameters. This allows the
...
in a non-dogmatic way.\label{tab:priors}}
\end{table*}
...
\begin{eqnarray}
f(S) &=& \sum_{N=0}^{10} \frac{1}{N}\sum_{i=1}^N p(S_i | N, \bdata).
\end{eqnarray}
...
this signal exists is 70\%, and given that it exists, its velocity amplitude
is about 15 m s$^{-1}$.
