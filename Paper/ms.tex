...
We ran the MCMC on the simulated dataset to obtain the posterior distribution
for the model parameters. The posterior distribution for $N$, the number of
...
results for all periods. Defining the log-periods by
...
recovered, with probability close to 1. One period which was actually present
was not detected because it had a very small amplitude. There is also some
evidence for periods which did not exist, however the posterior probabilities
for these peaks are not close to 1.
...
\caption{The posterior distribution for the periods\label{fig:fake_data_periods}}
\end{figure}

\begin{figure}
\includegraphics[scale=0.4]{Figures/fake_data_posterior.eps}
\caption{\label{fig:fake_data_posterior}}
\end{figure}
